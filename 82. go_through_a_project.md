...
- there are default, ```-ns``` (not small), ```-m``` (mediun), ```-l``` (large)
...
- send message to the server using certain method like:
  - fetch with the first parameter: the address
  - with the second parameter: an object that describe this fetch
  - method: get(default), post, put, delete
  - headers: {'Content-Type': 'application/json'},
  - body: what do we have, and we need to send it no by JS object but a JSON format.
```
