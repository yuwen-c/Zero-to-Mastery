...
- in async cases, action doesn't return an object, instead, it returns another function.
  - a thunk middlewear is needed here
  - to describe the promise, we have three types, states- pending, success, fail.
...
#### reducer.js
- the describing contants are needed.
...
- use switch case to list all situations and return a corresponding state.
- use Object.assign() method to create a new object state.
- a default case is also needed.
...
- we also need a ```applyMiddleware``` from redux to use it. so the final version of store would be:
  - ```const store = createStore(rootReducer, applyMiddlewear(thunkMiddleware, logger))
- if you have more then 1 reducer, then ```combineReducers``` is needed. use it to create a rootReducer
  - here we'll need the names of reducers, so import them.
  - use destructuting syntax: combineReducers({ reducerA, reducerB })

#### App.js 
- let App known thar Redux is working => connect
- import ```connect``` from react-redux
- at the bottom: export deafault ```connect(mapsStateToProps, mapsDispatchToProps)(App)```.
  - state: let App known which state change it should listening
  - Dispatch: let App known which action it should follow up
...
return{
	onSearchChange: ()
	onRequestRobots: () => actionB(dispatch) // OR: dispatch(actionB())  // it's a function that 
